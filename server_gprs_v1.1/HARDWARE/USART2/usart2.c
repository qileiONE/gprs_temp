...
{ 	
	...
	{	 			   
		...
		if(!USART2_RX_REC_ATCOMMAD)	
		{
			...
			USART2_RX_STA=0;
			if(strstr((char*)USART2_RX_BUF,"+IPD")!=NULL)	Flag_Rec_TCP=1;//¼ì²âµ½ÓĞÊı¾İ¹ıÀ´
		}		
	}   
}
//ÉèÖÃTIM4µÄ¿ª¹Ø
...
//		{
//		  TIM_SetCounter(TIM4,0);//¼ÆÊıÆ÷Çå¿Õ      
//			//if(USART3_RX_STA==0)TIM4_Set(1);	 	//Ê¹ÄÜ¶¨Ê±Æ÷4µÄÖĞ¶Ï 
...
{ 	
 ...
 //	USART2->BRR=(pclk1*1000000)/(bound);// ²¨ÌØÂÊÉèÖÃ	 
	//USART2->CR1|=0X200C;  	//1Î»Í£Ö¹,ÎŞĞ£ÑéÎ».
	USART_DMACmd(USART3,USART_DMAReq_Tx,ENABLE);  	//Ê¹ÄÜ´®¿Ú2µÄDMA·¢ËÍ
	UART_DMA_Config(DMA1_Channel7,(u32)&USART3->DR,(u32)USART3_TX_BUF);//DMA1Í¨µÀ7,ÍâÉèÎª´®¿Ú2,´æ´¢Æ÷ÎªUSART2_TX_BUF 
